...
class Meter:
    ...
    def update_gt(self, label, pred):
        if isinstance(label, np.ndarray):
            if label.ndim > 0 and len(label) > 0:
                label_val = label[0] if label.ndim == 1 else label.item()
            else:
                label_val = label.item() if hasattr(label, 'item') else float(label)
        else:
            label_val = label
        self.labels.append(np.clip(label_val, 0, 1))
    ...
    def pr_auc(self):
        """Average Precision (PR-AUC) computed on probability scores.
        Returns: float in [0,1]
        """
        # labels/preds are Python lists; convert to numpy arrays
        if len(self.labels) == 0:
            return 0.0
        try:
            return float(average_precision_score(np.array(self.labels), np.array(self.preds)))
        except Exception:
            return 0.0

    def precision_recall_f1(self, thres=None):
        """Return precision, recall, f1 under a given threshold (or optimal threshold).
        Values are floats in [0,1].
        """
        _, _, precision, recall, fscore, _ = five_scores(self.labels, self.preds, op_thres=thres)
